...
class Create extends Dcon  {
    ...
    function setConsignmentes($data)
    {



        $sql = "INSERT INTO `consignmentes` (`start_date`, `end_date`, `courier_name`, `status`, `file`,  `created_at`, `updated_at`) VALUES (?,?,?,?,?,?,?)";
        $is_true =  $this->connect()->prepare($sql)->execute($data);
       if($is_true)
       {
           $id = $this->GetData('consignmentes');
...
if (isset($_POST['status'])) {
    ...
    $is_save = $create->setConsignmentes($reqest);
